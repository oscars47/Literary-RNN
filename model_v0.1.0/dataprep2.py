...
class TextData:
    ...
    def format_data(self):
        ...
        for i in range(1, len(self.text)-2): # 3 is the min number of characters to add; but we must add 21 bc range object goes to len-1
            # if i is the last possible index, then len must be 3
            # t0: toast 0, t1: toast 1, h: honey (target)
            if i == 1: # starting at index 1
                toast_len=1
                t0 = self.text[0]# half 0
                t1 = self.text[2] # second half
                h = self.text[1] # target sits in middle
                sentences.append(t0+t1)
                target_chars.append(h)
            elif i == len(self.text)-3:
                toast_len=1
                t0 = self.text[-3]# half 0
                t1 = self.text[-1] # second half
                h = self.text[-2] # target sits in middle
                sentences.append(t0+t1)
                target_chars.append(h)
            else:
                ...
                if (toast_len > (len(self.text) - i)) or (toast_len > i): # need to check we dont over or try to go back father than we've come
                    tryagain=True
                    while tryagain==True: # need to keep subtracting by 1 and seeing if this works
                        toast_len-=1
                        if (toast_len <= (len(self.text) - i)) or (toast_len <= i):
                            tryagain=False
                            break
                # now we can append len 
                t0 = self.text[i-toast_len:i]
                t1 = self.text[i+1:i+toast_len]
                h = self.text[i]
                sentences.append(t0+t1)
                target_chars.append(h)
        ...
        # initialize all 0s
        data = np.zeros((len(complete_sentences), self.maxChar*2, len(self.alphabet)), dtype = np.bool_)
        labels = np.zeros((len(complete_sentences), len(self.alphabet)), dtype = np.bool_)
